 ...
 *
 *  @author neal sharma
 *  I worked closely with Taylor Lee, Krishna Ravi, Marco Gellecanao,
 *  Naveen Bahadur, Mike Chou
 *
 */
class Model implements Iterable<Model.Sq> {
    ...
    Model(int[][] solution) {
        ...
        deepCopy(solution, _solution); _solnNumToPlace
                = new Place[last + 1];
        ...
        for (int x = 0; x < _width; x++) {
            for (int y = 0; y < _height; y++) {
                if (sequenceNumbers.contains(_solution[x][y])) {
                    int k = sequenceNumbers.indexOf(_solution[x][y]);
                    sequenceNumbers.remove(k);
                }
            }
        }
    ...
    /**
     *  This checks the model for all square numbers being there.
     */
    void squarecheck() {
        for (Sq currentSq: _allSquares) {
            if (currentSq._successors == null) {
                currentSq._successors = new PlaceList();
            }
            if (currentSq._predecessors == null) {
                currentSq._predecessors = new PlaceList();
            }
        }
        for (Sq currentSq: _allSquares) {
            for (Place succ: allSuccessors(currentSq.pl, currentSq._dir)) {
                if (currentSq.connectable(get(succ))) {
                    currentSq._successors.add(succ);
                }
            }
            if (currentSq._successors != null) {
                for (Place places : currentSq._successors) {
                    Sq otherSquare = get(places);
                    otherSquare._predecessors.add(currentSq.pl);
                }
            }
        }
    }
    ...
    Model(Model model) {
        ...
        for (int x = 0; x < _width; x++) {
            for (int y = 0; y < _height; y++) {
                Sq elem = new Sq(model._board[x][y]);
                _board[x][y] = elem;
                _allSquares.add(elem);
            }
        }
        for (Sq copysquare: _allSquares) {
            for (Sq origsquare: model._allSquares) {
                if (copysquare.pl == origsquare.pl) {
                    if (origsquare.successor() == null) {
                        copysquare._successor = null;

                    } else {
                        Place origSuccessorPlace = origsquare.successor().pl;
                        copysquare._successor =
                                get(origSuccessorPlace);
                    }
                    if (origsquare.predecessor() == null) {
                        copysquare._predecessor = null;

                    } else {

                        Place origPredecessorPlace
                                = origsquare.predecessor().pl;
                        copysquare._predecessor =
                                get(origPredecessorPlace);
                    }
                    if (origsquare.head() == null) {
                        copysquare._head = null;

                    } else {
                        Place origHeadPlace = origsquare.head().pl;
                        copysquare._head =
                                get(origHeadPlace);
                    }
                }
    ...
    /** Connect all numbered cells with successive numbers that are
    ...
    boolean autoconnect() {
        ...
        for (Sq square: _allSquares) {
            for (Sq square2: _allSquares) {
                if ((square != square2) && square.sequenceNum() != 0
                        && square2.sequenceNum() != 0
                        && (square.connectable(square2))) {

                    square.connect(square2);
                    flag = true;
                }
            }
        }
    ...
    /** Sets the numbers in my Squares to the solution from which this was
    ...
    private int arrowDirection(int x, int y) {
        ...
        if (seq0 == _width * _height) {
            ...
            Place successorPlace = solnNumToPlace(seq0 + 1);
            return dirOf(x, y, successorPlace.x, successorPlace.y);
        }
    }
    ...
    final class Sq {
        ...
        boolean connect(Sq s1) {
            ...
            if (_sequenceNum != 0) {
                ...
                while (pointer._successor != null) {
                    pointer._successor._sequenceNum = pointer._sequenceNum + 1;
            ...
            if (s1._sequenceNum != 0) {
                ...
                while (pointer._predecessor != null) {
                    pointer._predecessor._sequenceNum
                            = pointer._sequenceNum - 1;
                    pointer = pointer._predecessor;
                }
            }

            Sq pointerHead = this;
            while (pointerHead._successor != null) {
                pointerHead._successor._head = _head;
                pointerHead = pointerHead._successor;
            }
        ...
        void disconnect() {
            ...
            if (_sequenceNum == 0) {
                if (this.group() == -1 && next.group() == -1) {
                    releaseGroup(_group); releaseGroup(next._group);
                    _group = -1; next._group = -1;
                } else if (this.group() == -1) {
                    _group = -1;
                } else if (next.group() == -1) {
                    next._group = -1;
                } else {
                    next._group = newGroup();
                }
            } else {
                Sq pointer = this; ArrayList<Sq> preceding = new ArrayList<>();
                boolean precedingFlag = false;
                while (pointer != null && pointer._group == _group) {
                    preceding.add(pointer);
                ...
                if (precedingFlag) {
                    if (this._predecessor != null) {
                        for (Sq square : preceding) {
                            square._sequenceNum = 0; square._group = newGroup();
                        }
                    } else {
                        for (Sq square: preceding) {
                            square._sequenceNum = 0; square._group = -1;
                        }
                    }
                }
                Sq pointer2 = next;
                ArrayList<Sq> suc = new ArrayList<>(); boolean sFlag = false;
                while (pointer2 != null && pointer2._group == next._group) {
                    suc.add(pointer2);
                    if (!pointer2.hasFixedNum()) {
                        sFlag = true;
                    }
                    pointer2 = pointer2._successor;
                }
                if (sFlag) {
